...
const emailZodeSchema=z.string({
    required_error:"email must be required",
    invalid_type_error:"envalid email format"
}).trim().email({message:"invalid email"}).min(12,{message:"email is too short"})

const passwordZodSchema=z.string({
    required_error:"password must be required",
    invalid_type_error:"password must be string"
}).trim().min(8,{message:"password length must be >8 character"}).max(15,{message:"password length must be <15 character"})

const roleZodeSchema=z.string({
    required_error:"role must be required",
    invalid_type_error:"invalid role type"
}).trim().optional()

const dobZodSchema=z.string({
    required_error:"dob must be required",
    invalid_type_error:"dob must be string"
}).trim().min(1,{message:"dob must be 1 letter"})

const genderZodSchema=z.string({
    required_error:"gender must be required",
    invalid_type_error:"gender must be string"
}).trim().min(1,{message:"gender must be 1 letter"})

const mobileZodSchema=z.string({
    required_error:"mobile must be required",
    invalid_type_error:"mobile must be string"
}).trim().min(1,{message:"mobile must be 1 letter"})

const fullnameZodSchema=z.string({
    required_error:"name must be required",
    invalid_type_error:"name must be string"
}).trim().min(4,{message:'name must be 4 character'}).max(25,{message:"name must be less than 25 character"})
...
export const UserSignUpZodSchema=z.object({
    ...
    }).trim().min(4,{message:'username must be 4 character'}).max(25,{message:"username must be less than 25 character"}),
...
}).refine((data)=>data.newPassword===data.confirmPassword,{message:"password not match",path:["confirmPassword"]})
...
export const SellerUserZodSchema=z.object({
    ...
    }).trim().min(1,{message:"postalCode must be 1 letter"}),
    gender:genderZodSchema,
})  